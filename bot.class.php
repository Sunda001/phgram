...
class Bot {
	...
	**/
	...
	/**
	* If true, you will bw notified when the request fails. Set the $debug_admin to your id
	*
	* @var bool $debug
	* @var $debug_admin
	**/
	...
	*/
	public function __construct(string $bot_token, $debug = false) {
		$this->bot_token = $bot_token;
		$this->data = $this->getData();
		$this->update_type = @array_keys($this->data)[1];
		$this->debug_admin = $this->ChatID();
		$this->debug = $debug;
	}
	...
	*/
	public function __call(string $method, array $args = NULL) {
		...
		if (!$reply['ok'] && $this->debug) {
			$this->send(json_encode($reply), ['chat_id' => $this->debug_admin]);
		}
		return $reply;
	}
	...
	*/
	...
	public function Language() {
		$type = $this->update_type;
		if (isset($this->data[$type]['from']['language_code'])) {
			return $this->data[$type]['from']['language_code'];
		} elseif (isset($this->data[$type]['message']['from']['language_code'])) {
			return $this->data[$type]['message']['from']['language_code'];
		}
	...
	public function Entities() {
		$type = $this->update_type;
		if (isset($this->data[$type]['entities'])) {
			return $this->data[$type]['entities'];
		} elseif (isset($this->data[$type]['message']['entities'])) {
			return $this->data[$type]['message']['entities'];
		}
